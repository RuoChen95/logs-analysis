...
from log group by path order by num desc) as logs
...
and
articles.author = authors.id) as logs
group by logs.name
order by sum desc
""")
results = c.fetchall()
...
a.day as date,
round(
100 * cast((b.hits) as decimal)/cast(a.hits + b.hits as decimal)
, 2)
as errors
from
...
join
(select
date(time) as day, count(*) as hits
from log
